...
class WebLayerTest {
	...
	@Test
	void healthIsUpWithTheDatabase() throws Exception {
	...
	@Test
	void requestsThatAreNotJsonAreRefused() throws Exception {
		mvc.perform(post("/test/echo").contentType(MediaType.APPLICATION_FORM_URLENCODED).content("phone=x"))
			.andExpect(status().isUnprocessableContent())
			.andExpect(jsonPath("$.error.message").value("Send the request as JSON."));
	}

	@Test
	void anIdThatIsNotAnIdFindsNothing() throws Exception {
		mvc.perform(get("/test/things/not-an-id"))
			.andExpect(status().isNotFound())
			.andExpect(jsonPath("$.error.code").value("not-found"));
	}

	@Test
	void otherSitesCannotChangeThingsThroughTheBrowser() throws Exception {
		mvc.perform(post("/test/echo").header("Origin", "https://evil.example").contentType(MediaType.APPLICATION_JSON).content("{\"phone\":\"x\"}"))
			.andExpect(status().isForbidden());
		mvc.perform(post("/test/echo").header("Origin", "http://localhost:3000").contentType(MediaType.APPLICATION_JSON).content("{\"phone\":\"x\"}"))
			.andExpect(status().isOk());
	}

	@Test
	void theWebAppMayCallFromTheBrowser() throws Exception {
		mvc.perform(options("/auth/session").header("Origin", "http://localhost:3000").header("Access-Control-Request-Method", "GET"))
	...
	void otherSitesMayNot() throws Exception {
		mvc.perform(options("/auth/session").header("Origin", "https://evil.example").header("Access-Control-Request-Method", "GET"))
	...
	static class Probes {
		...
		static class ProbeController {
			...
			@GetMapping("/test/things/{id}")
			String thing(@PathVariable UUID id) {
				return id.toString();
			}
